...
输入六位股票代码后读取已缓存行情；用户主动刷新时访问公开 Provider，展示价格、日内行情、公告资讯、来源和时间。主源失败时尝试备用源，全部失败时保留旧快照。
...
> 我做了一个 React + FastAPI + PostgreSQL 的可部署 A 股投研应用 AlphaLens。前端有市场概览、在线市场、因子选股、AI 投研、资金监控、个股对比、每日复盘、行业轮动和设置九个页面；后端采用 Route、Service、Repository 分层，PostgreSQL 保存种子快照、刷新数据、报告和任务状态。
>
