...
1. **数据层**：将多因子结果、行业轮动和研报作为种子数据导入 PostgreSQL；市场数据按“保存快照优先、手动刷新、失败回退”的原则处理。
...
- 因子页可在线更新当前候选池的动量、换手率变化和波动率，并结合最近财务因子重新排名；这不是全市场重新选股。
- 财务因子随财报更新，不应被描述为盘中实时数据；全量股票池重算通过管理员 CLI 完成。
- 全市场概览主源不可用时，会降级为“四大指数 + 当前 30 只研究候选股”真实行情，并明确标注统计范围。
