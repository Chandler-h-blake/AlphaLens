...
export function SettingsPage() {
  ...
  return <div className="page-content">
    ...
    <section className="content-panel"><h2>快照新鲜度</h2><div className="distribution">{Object.entries(status.snapshots).map(([name, time]) => <div key={name}><span>{name === 'dashboard' ? '市场概览' : '资金监控'}</span><strong>{time ? time.slice(0, 16).replace('T', ' ') : '未初始化'}</strong></div>)}</div></section>
